...
public class LibraryManager
{
    ...
    {
        get => Environment.GetFolderPath(Environment.SpecialFolder.MyMusic);
    }
    ...
    private bool AddToLibrary(string path)
    {
        lock (_songsLock)
        {
            ...
            if (song is null)
                return false;
            if (Songs.TryAdd(song.path, song) == false)
                return false;
        }
    ...
    private bool RemoveFromLibrary(string path)
    {
        lock (_songsLock)
        {
            ...
            if (song is null)
                return false;
            if (Songs.Remove(song.path) == false)
                return false;
        }
    ...
    private bool RenameInLibrary(string oldPath, string newPath)
    {
        lock (_songsLock)
        {
            ...
            if (song is null)
                return false;
            if (Songs.Remove(oldPath) == false)
                return false;
            if (Songs.TryAdd(newPath, song) == false)
                return false;
        }
